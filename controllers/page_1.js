  ...
  try {
    ...
    const Batch_Details = Batches.map((item) => {
      ...
      return {
        ...
        ToT_fields_present: "",
        assessment_done: assessment_done,
        tot_certified: tot_certified,
        tot_assessment: tot_assessment * 4,
    ...
    const result = master_trainers2.map((item) => {
      let tot_certified = 0;
      let tot_assessment = 0;
      let assessment_done = 0;
      const filterData = dat2.filter((itemTask) => {
        if (item._id.toString() == itemTask.coachId.toString()) {
          tot_certified = tot_certified + itemTask.tot_certified;
          tot_assessment = tot_assessment + itemTask.tot_assessment;
          assessment_done = assessment_done + itemTask.assessment_done;
          return true;
        }
      });
      ...
      return {
        ...
        zone: item.zone,
        No_batch: filterData.length,
        tot_certified: tot_certified,
        tot_staff: tot_assessment / 4,
        assess_pend: tot_assessment - assessment_done,
